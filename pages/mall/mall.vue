...
<script>
import { getUser, getCategory, getProduct, getRecommend, getUserInfo } from '@/api/mall.js';
...
export default {
	...
	methods: {
		init() {
			...
			getUser().then(
				result => {
					const { integral, orderStatusNum } = result;
					this.userInfo = { ...this.userInfo, integral, complete_count: orderStatusNum.evaluated_count };
				},
				err => {}
			);
			getUserInfo().then(
				result => {
					...
					this.userInfo = { ...this.userInfo, avatar: avatarUrl, nickname: nickName };
				},
				err => {}
			);
		},
