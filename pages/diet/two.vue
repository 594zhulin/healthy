<template>
	<view class="page-two">
		...
		<view class="btn-content">
			...
			<view class="next-btn" @click="navigateTo('/pages/diet/three')">下一步</view>
		</view>
	</view>
</template>
...
export default {
	...
	onLoad() {
		getSurvey({ is_model: 2 }).then(
			result => {
				this.question = result;
				result.map(item => {
					let obj = { questionnaire_id: item.question_id, is_model: 2, answer: [item.option[0].option_id] };
	...
	methods: {
		...
		navigateTo(url) {
			submitSurvey({ is_model: 2, data: JSON.stringify(this.answer) }).then(
				result => {
					uni.navigateTo({
						url
					});
