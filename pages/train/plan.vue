<template>
	<view class="page-plan">
		<view class="radar-content">
			...
			<view class="echarts">
				<mpvue-echarts ref="radarChart" :echarts="echarts" @onInit="initRadarChart" />
			</view>
		</view>
		<view class="train-content">
			<view class="today-item">
				<view class="title">今日</view>
				<view class="step">{{ yesterday_step_num }}万</view>
		...
		<view class="trend-content">
			...
			<view class="echarts">
				<mpvue-echarts ref="lineChart" :echarts="echarts" @onInit="initLineChart" />
			</view>
		</view>
		<view class="desc-content">
			<view class="title">AI智能训练简介</view>
			<image class="image" :src="AI_drill_picture" mode="widthFix"></image>
			<view class="text">{{ AI_drill_intro }}</view>
		</view>
		...
		onLoad() {
					...
					wx.getWeRunData({
						success(infoRes) {
							...
							getStep({
								...
								result => {
									_this.yesterday_step_num = result.stepInfoList[result.stepInfoList.length - 1].step;
								},
								err => {}
							);
						}
			...
			getTrainData().then(
				result => {
					...
					_this.deposit_num = deposit_num || 0;
					_this.pliable = pliable;
					_this.force = force;
					_this.aerobics = aerobics;
					_this.leisure = leisure;
					_this.AI_drill_picture = AI_drill_picture;
					_this.AI_drill_intro = AI_drill_intro;
				},
				err => {}
			);
			getFitnessData().then(
				result => {
					_this.initLineChartOption(result.sheet);
					const {
						bodily_type,
						grip_num,
						sit_reach,
					...
					_this.initRadarChartOption([bodily_type, grip_num, sit_reach, foot_closed, response_at]);
					_this.create_at = result.lately_data.create_at;
				},
				err => {
					uni.showToast({
						icon: 'none',
