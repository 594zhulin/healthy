		...
		<view class="list-content">
			<view class="list-item" v-for="item in order" :key="item.id" @click="navigateTo('/pages/order/detail?id=' + item.order_id)">
				<view class="title">
					...
					<view class="status" v-if="item._status._type == 1">待发货</view>
					<view class="status" v-if="item._status._type == 2">待收货</view>
					<view class="status" v-if="item._status._type == 3">已签收</view>
				</view>
				<view class="product" v-for="cart in item.cartInfo" :key="cart.id">
					...
					<view class="detail">
						...
						<view class="step" v-if="cart.productInfo.is_model == 1">{{ cart.productInfo.buy_credits >= 1000 ? (cart.productInfo.buy_credits / 10000).toFixed(2) + '万' : cart.productInfo.buy_credits }}</view>
