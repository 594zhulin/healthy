<template>
	<view class="page-launch">
		<swiper class="swiper" :autoplay="true" interval="5000">
			<swiper-item class="swiper-item" v-for="item in banner" :key="item.events_id"><image class="banner" :src="slide_url"></image></swiper-item>
		</swiper>
...
<script>
import { getBanner, login, loginAuth, setUserInfo, getLoginInfo, getPhone, start, getFaultList, repair } from '@/api/launch.js';
export default {
	props: {
		device_id: String
	},
	...
	methods: {
		...
		start() {
			start({ device_id: this.device_id }).then(
				result => {
					this.boot = true;
		...
		submit() {
			repair({ device_id: this.device_id, faultcause_id: this.faultcause_id }).then(
				result => {
					this.$refs['repairModal'].close();
				},
				err => {
