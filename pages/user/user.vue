...
<script>
import { getUser, getScore, getRisk, getScoreList, getAddress, getUserInfo } from '@/api/user.js';
		...
		init() {
			...
			getUser().then(
				result => {
					...
					_this.user = { ..._this.user, integral, orderStatusNum };
				},
				err => {}
			);
			getUserInfo().then(
				result => {
					...
					_this.user = { ..._this.user, avatar: avatarUrl, nickname: nickName };
				},
				err => {}
			);
			getScore().then(
