	...
	<view class="page-home">
		<template>
			<view class="overview-content">
				...
				<view class="bank-item" @click="navigateToBank('/pages/bank/bank?step=' + no_deposit_num + '&total=' + integral_num)">
					...
					<view class="text" :class="{ opacity: !isLogin }">待存入</view>
					<view class="step">{{ isLogin ? no_deposit_num + '万步' : '请登录' }}</view>
					<view class="text total" :class="{ opacity: !isLogin }">共累计{{ integral_num }}万步</view>
...
import { login, signIn, getScore, getUserStep, getStep, getProduct, getLastTime, cacheStep } from '@/api/home.js';
export default {
	components: {
		mpvueEcharts
	},
	...
	methods: {
		...
		openConfirm() {
			wx.showModal({
				content: '检测到您没打开体能观测的微信运动步数权限，是否去设置打开？',
				confirmText: '确认',
				cancelText: '取消',
